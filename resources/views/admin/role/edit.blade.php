...
<div class="card shadow">
    <div class="text-center mt-4">
        <h1 class="h4 text-gray-900 mb-4">Chỉnh sửa Vai trò!</h1>
    </div>
    <form class="user" action="{{ route('roles.update', ['id' => $role->id]) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        @if (!empty($role))
        ...
        <div class="text-dark card-body border-top">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="display_name" rows="4">{{ old('display_name', $role->display_name ?? '') }}</textarea>
                <span id="name-error" style="color: red;"></span>
            </div>
